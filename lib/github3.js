...
Github3.prototype.createRefHead = function (repo, name, branch, commit_sha, callback) {
  var ref = {'sha' : commit_sha, 'ref' : 'refs/heads/' + branch};
  return this._post('/repos/'+name+'/'+repo+'/git/refs', JSON.stringify(ref), callback);
};
Github3.prototype.updateRefHead = function (repo, name, branch, commit_sha, force, callback) {
  var ref = {'sha' : commit_sha, 'force' : force};
  return this._patch('/repos/'+name+'/'+repo+'/git/refs/heads/'+ branch, JSON.stringify(ref), callback);
};
